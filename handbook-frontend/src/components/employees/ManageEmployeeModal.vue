    ...
    <div class="modal-overlay">
        <div class="modal">
            ...
            <div class="modal-body">
                <form @submit.prevent="$emit('update', employee)">
                    <label>
                        * ФИО:
                        <input type="text" v-model="employee.name" required />
                    </label>
                    <Dropdown :options="prepareDetachedStructure(departments)" v-model:modelValue="employee.department"
                        :default-label="`Выберите отдел`" />
                    <Dropdown :options="professions" v-model:modelValue="employee.profession"
                        :default-label="`Выберите профессию`" />
                    <label>
                        Примечание:
                        <input type="text" v-model="employee.note" />
                    </label>
                    <div class="modal-actions">
                        <button type="submit" class="save-button">Сохранить</button>
                        <button @click.prevent="$emit('delete', employee)" class="delete-button">Удалить</button>
                    </div>
                </form>
            </div>
        </div>
